...
async function testImageProxy() {
  devLog('🧪 Testing Image Proxy API...');
  ...
  try {
    devLog('Testing image proxy with Camp Origin URL...');
    const proxyUrl = `/api/image-proxy?url=${encodeURIComponent(campOriginUrl)}`;
    devLog('Proxy URL:', proxyUrl);
    ...
    if (response.ok) {
      ...
      devLog('✅ Image proxy successful!');
      devLog('Blob size:', blob.size, 'bytes');
      devLog('Blob type:', blob.type);
      ...
      devLog('✅ Test image added to page');
...
async function testDirectImageFetch() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      document.body.appendChild(img);
      devLog('✅ Direct test image added to page');
    }
