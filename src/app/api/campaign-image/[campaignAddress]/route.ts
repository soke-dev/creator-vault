...
export async function GET(
  ...
  try {
    ...
    console.log(`[API] Fetching image for campaign: ${campaignAddress}`);
    
    if (!campaignAddress) {
      console.log('[API] Error: Campaign address is missing');
    ...
    if (cached && Date.now() - cached.timestamp < CACHE_DURATION) {
      console.log(`[API] Returning cached result for campaign: ${campaignAddress}`);
      return NextResponse.json(cached.data);
    }

    // Check rate limiting
    if (isRateLimited(campaignAddress)) {
      console.log(`[API] Rate limited for campaign: ${campaignAddress}`);
      return NextResponse.json(
    ...
    if (!imageRecord) {
      console.log(`[API] No image found for campaign: ${campaignAddress}`);
    ...
    console.log(`[API] Image found for campaign: ${campaignAddress}`);
    ...
    if (imageRecord.image_file) {
      ...
      if (fileUrl) {
        console.log(`[API] Using PocketBase file URL: ${fileUrl}`);
        imageUrl = fileUrl;
      }
    }
    ...
    if (imageUrl && imageUrl.includes('amazonaws.com')) {
      console.log(`[API] AWS S3 URL detected - suggesting proxy to avoid expiration/CORS issues`);
      suggestProxy = true;
    }
